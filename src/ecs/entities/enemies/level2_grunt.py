# src/ecs/entities/enemies/level2_grunt.py
import arcade
...
class Level2Grunt(arcade.Sprite):
    """二级步兵实体（包含站立和跑步动画）"""
    _json_cache = None

    @staticmethod
    def preload_json():
        if Level2Grunt._json_cache is None:
            config_path = os.path.join(os.path.dirname(__file__), "level2_grunt.json")
            with open(config_path, "r", encoding="utf-8") as f:
                Level2Grunt._json_cache = json.load(f)

    def __init__(self):
        super().__init__()
        self.logger = logger.getChild("Level2Grunt")
        # 优先使用缓存
        if Level2Grunt._json_cache is None:
            Level2Grunt.preload_json()
        config = Level2Grunt._json_cache
        ...
        self.logger.info("level2 grunt initialized")
    ...
    def load_textures(self):
        ...
        try:
            ...
            level2_grunt_path = get_asset_path("enemies/level2_grunt")

            # 加载攻击动画帧
            self.attack_textures = [
                arcade.load_texture(os.path.join(level2_grunt_path, "attack_1.png")),
                arcade.load_texture(os.path.join(level2_grunt_path, "attack_2.png"))
            ]
