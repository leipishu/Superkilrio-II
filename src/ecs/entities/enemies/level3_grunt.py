# src/ecs/entities/enemies/level3_grunt.py
import arcade
from PIL.Image import FLIP_LEFT_RIGHT
from src.constants import *
from src.utils.logging_config import logger
from src.ecs.systems.ai_system import AISystem  # 新增导入
from src.items.registry import item_registry
import random
import json


class Level3Grunt(arcade.Sprite):
    """二级步兵实体（包含站立和跑步动画）"""
    _json_cache = None

    @staticmethod
    def preload_json():
        if Level3Grunt._json_cache is None:
            config_path = os.path.join(os.path.dirname(__file__), "level3_grunt.json")
            with open(config_path, "r", encoding="utf-8") as f:
                Level3Grunt._json_cache = json.load(f)

    def __init__(self):
        super().__init__()
        self.logger = logger.getChild("Level3Grunt")
        # 优先使用缓存
        if Level3Grunt._json_cache is None:
            Level3Grunt.preload_json()
        config = Level3Grunt._json_cache
        ...
        self.logger.info("level3 grunt initialized")
    ...
    def load_textures(self):
        ...
        try:
            ...
            level3_grunt_path = get_asset_path("enemies/level3_grunt")

            # 加载攻击动画帧
            self.attack_textures = [
                arcade.load_texture(os.path.join(level3_grunt_path, "attack_1.png")),
                arcade.load_texture(os.path.join(level3_grunt_path, "attack_2.png")),
                arcade.load_texture(os.path.join(level3_grunt_path, "attack_3.png")),
            ]
    ...
    def destroy(self):
        try:
            from src.items.weapons.firelock import Firelock
            from src.items.dropped_item import DroppedItem

            # 测试物品创建
            self.logger.info("Creating IronHook instance...")
            firelock = Firelock()
            self.logger.info(f"hook.texture exists: {hasattr(firelock, 'texture')}")
            self.logger.info(f"hook.texture is None: {firelock.texture is None}")

            if not hasattr(firelock, 'texture') or firelock.texture is None:
